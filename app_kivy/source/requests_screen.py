...
class RequestsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.add_header_label(header_layout, "[b]Requestname[/b]")
        ...
        # Load Requests
        self.load_requests_from_db(SELECT_ALL_USERS)
        ...
        # Back from requests to users Layout
        self.back_button = Button(text="Back to Fields")
        self.back_button.bind(on_press=self.go_to_users_screen)
        
        # Add user Layout
        self.add_request_button = Button(text="Add Request")
        self.add_request_button.bind(on_press=self.add_field_request)
        
        btn_layout.add_widget(self.back_button)
        btn_layout.add_widget(self.add_request_button)
    ...
    def add_field_request(self, instance):
        ...
        key_input = TextInput(hint_text="Key name")

        def _save_user(instance):
            record = (key_input.text, )
            row = SelectableRow(
                *record,
        ...
        popup = Popup(title="Add Request", content=popup_layout, size_hint=(0.7, 0.4))
        popup.open()
